...
export function isTenOrAce(rank: Rank): boolean {
  return rank === "A" || ["K", "Q", "J", "10"].includes(rank);
}

export function dealerUpcardNeedsPeek(upcard: Card): boolean {
  return isTenOrAce(upcard.rank);
}

/** Total usando todas las cartas (incl. ocultas) — para lógica interna del crupier */
...
export function isSoft17(cards: Card[]): boolean {
  const total = handTotalAll(cards);
